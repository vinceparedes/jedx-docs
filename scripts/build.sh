...
rm -rf "${DOCS}"
mkdir -p "${DOCS}" "${ASSETS}/images" "${ASSETS}/pdfs" "${ASSETS}/schemas" "${ASSETS}/zips" "${DOCS}/sam-templates" "${DOCS}/ai-generated"
...
# ------------------------------------------------------------
# Schemas (D_Schemas): individual downloadable files
# ------------------------------------------------------------
...
def extract_title_desc(p):
    ...
    # JEDx schemas wrap the real schema in `definitions.<key>`.
...
# ------------------------------------------------------------
# Sample Data (E_SampleData): single ZIP + README rendering
# ------------------------------------------------------------
...
if readme.exists():
    ...
    # Drop the source's H1 if present so we can supply our own title.
